 ...
 * bg_fen : point haut gauche dans la fenetre où afficher le morceau d'image
 * bg_morceau : point haut gauche du morceau à afficher (relativement à l'image)
 * (w_morceau,h_morceau) : dimensions du morceau à afficher
 */
void affiche_morceau_image(char *nomfic, POINT bg_fen, POINT bg_morceau,int w_morceau, int h_morceau)
{
	SDL_Surface *image = NULL;
	...
	if (image == NULL) fprintf(stderr,"Impossible charger l'image : %s\n",nomfic);
	/* On blitte par-dessus l'écran */
	srect.x = bg_morceau.x; srect.y = image->h-bg_morceau.y-h_morceau;
	srect.w = w_morceau; srect.h = h_morceau;
	
	drect.x = bg_fen.x; drect.y = HEIGHT-bg_fen.y-h_morceau;
	drect.w = w_morceau; drect.h = h_morceau;
	
	SDL_BlitSurface(image, &srect, SDL_screen, &drect);
